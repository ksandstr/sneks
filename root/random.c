 ...
 * full strength for short sequences and when good entropy was available.
 * this algorithm was invented at random as a substitute for a proper thing
 * that looks right under test, so it should be replaced with something real
 * by the time sneks gets any access control at all.
 */
void generate_u(void *outbuf, size_t length)
{
	...
	while(done < length) {
		...
		int n = min_t(int, length - done, buffer_depth);
