...
static void load_spec(int *offset, int *mrpos, const char *name, L4_Word_t meta)
{
	if(--*offset < 0) {
...
static void describe_selftests(L4_ThreadId_t sender, int offset)
{
	...
	for(size_t i=0; i < n_specs && mrpos < 58; i++) {
		 ...
		 * don't use those.
		 */
		char *path = strdupa(specs[i]->path), *sep = strchr(path, ':');
		if(sep != NULL) *sep = '\0';
		if(!streq(prefix, path)) {
			strscpy(prefix, path, sizeof prefix);
			load_spec(&offset, &mrpos, prefix, 1);
		}
		if(sep != NULL && !streq(group, sep + 1)) {
			strscpy(group, sep + 1, sizeof group);
			load_spec(&offset, &mrpos, group, 2);
		}
		/* (priority is 0xff at bitpos 26) */
		load_spec(&offset, &mrpos, specs[i]->test->name,
			(specs[i]->test->iter_low & 0xff) << 2 | (specs[i]->test->iter_high & 0xffff) << 10);
	}
	L4_LoadMR(0, (L4_MsgTag_t){ .X.u = mrpos }.raw);
	L4_MsgTag_t tag = L4_Reply(sender);
	if(L4_IpcFailed(tag)) fprintf(stderr, "%s: reply failed: %s\n", __func__, stripcerr(L4_ErrorCode()));
	autodata_free(specs);
}
...
static void run_selftest(L4_ThreadId_t sender, int iter, const char *name)
{
	...
	struct wrapctx ctx = { .caller = sender, .test = L4_Myself(), .oldout = stdout, .olderr = stderr };
	FILE *wrapout = fopencookie(&ctx, "wb", (cookie_io_functions_t){ .write = &wrap_write, .close = &wrap_close });
	if(wrapout == NULL) {
		fprintf(stderr, "%s: fopencookie() failed: %s\n", __func__, strerror(errno));
		return;
	}
	L4_LoadMR(0, 0); L4_Reply(sender);
	FILE *oldin = stdin;
...
bool selftest_handling(L4_Word_t status)
{
	...
	L4_Word_t iter, offset, name[L4_UntypedWords(tag)];
	switch(L4_Label(tag)) {
		default: return false;
		case 0xeffe: L4_StoreMR(1, &offset); describe_selftests(sender, offset); return true;
		case 0xeffd:
			L4_StoreMR(1, &iter); L4_StoreMRs(2, L4_UntypedWords(tag) - 1, name);
