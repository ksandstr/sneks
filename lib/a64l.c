...
static int decode_byte(unsigned c, bool *fail) {
	if(c < ARRAY_SIZE(dec_table) && (dec_table[c] > 0 || c == '.')) return dec_table[c];
	else { *fail = true; return -1; }
}

static int encode_byte(int b) {
	static const char enc_table[64] = "./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	if(b >= 0 && b < ARRAY_SIZE(enc_table)) return enc_table[b]; else return '&'; /* (non-posix error indicator) */
}

long a64l(const char *str64) {
	long acc = 0;
	bool fail = false;
	for(int i = 0, scale = 0; i < 6 && str64[i] != '\0' && !fail; i++, scale += 6) acc |= decode_byte(str64[i], &fail) << scale;
	return fail ? 0 : acc;
}

char *l64a(long input) {
	static char str[7]; /* TODO: return a lib-wide TSS tempstr */
	unsigned v = input;
	for(int i = 0; i < 6 && v > 0; i++, v >>= 6) { str[i] = encode_byte(v & 0x3f); str[i + 1] = '\0'; }
	return str;
}
