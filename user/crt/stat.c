...
int fstat(int fd, struct stat *statbuf)
{
	...
	if(ext != NULL) {
		*statbuf = ext->st;
		...
		if(n == 0) __convert_statbuf(statbuf, &st);
		return NTOERR(n);
	}
}
...
int fstatat(int dirfd, const char *pathname, struct stat *statbuf, int flags)
{
	...
	struct resolve_out r; int n = __resolve(&r, dirfd, pathname, flags);
	if(n == 0) {
		struct sneks_path_statbuf tmp;
		if(n = __path_stat_object(r.server, r.object, r.cookie, &tmp), n == 0) __convert_statbuf(statbuf, &tmp);
	}
	return NTOERR(n);
}
