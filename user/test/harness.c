...
static int cmp_utest_ptr(const void *ap, const void *bp)
{
	...
	const struct utest *a = *(struct utest **)ap,
		*b = *(struct utest **)bp;
...
static void assign_test_ids(void *talctx, struct utest **sts, size_t n_sts)
{
	...
	{
		struct utest **s_index = bsearch(&px->body, sts,
			n_sts, sizeof *sts, &cmp_utest_ptr);
		assert(s_index != NULL);
		...
		for(int grp_ix=0; grp_ix < groups.size; grp_ix++) {
			...
			struct utest **g_index = bsearch(&g->body, s_index,
				p_size, sizeof *sts, &cmp_utest_ptr);
			assert(g_index != NULL);
...
static struct utest *parse_utest(
	void *tal, const struct utest_spec *spec)
{
...
static struct utest **get_tests(size_t *num_p, bool want_ids)
{
	...
	if(darray_empty(all_tests)) {
		...
		for(size_t i=0; i < n_specs; i++) {
			darray_push(all_tests, parse_utest(tal, specs[i]));
		}
		assert(all_tests.size == n_specs);
		qsort(all_tests.item, all_tests.size,
			sizeof(struct utest *), &cmp_utest_ptr);
	}
...
void run_test_by_spec(char **specs, size_t n_specs)
{
	...
	for(size_t i=0; i < n_specs; i++) {
		...
		struct utest *t = htable_get(&by_id, hash_string(spec),
