...
#define IN_TEST_MAGIC 0xb33b7007
...
{
	const struct systest *a = *(struct systest **)ap, *b = *(struct systest **)bp; /* const correctness is a bourgeois concept */
	int d;
...
bool in_test(void) { return tss_get(in_test_key) == (void *)IN_TEST_MAGIC; }
...
static void assign_test_ids(void *talctx, struct systest **sts, size_t n_sts)
{
	...
	for(struct idcomp *px = htable_first(top, &it); px != NULL; px = htable_next(top, &it)) {
		struct systest **s_index = bsearch(&px->body, sts, n_sts, sizeof *sts, &cmp_systest_ptr);
		assert(s_index != NULL);
...
static struct systest *parse_systest(void *tal, const struct systest_spec *spec)
{
...
static void run_systask_selftest(int iter)
{
	...
	diag("[wrapper running `%s' on %lu:%lu]", current_systest->name, L4_ThreadNo(inf->tid), L4_Version(inf->tid));
	int name_words = (strlen(current_systest->name) + sizeof(L4_Word_t) - 1) / sizeof(L4_Word_t);
	L4_LoadMR(0, (L4_MsgTag_t){ .X.u = 1 + name_words, .X.label = 0xeffd }.raw);
	L4_LoadMR(1, iter);
	...
	if(L4_IpcFailed(tag) || L4_Label(tag) == 1) {
		...
		plan_skip_all("IPC failed, ec=%lu, code=%#lx", L4_ErrorCode(), code);
		return;
	}
	/* passthru output from test process. */
	char iobuf[513];
	L4_MsgBuffer_t msgbuf; L4_MsgBufferClear(&msgbuf);
	L4_MsgBufferAppendSimpleRcvString(&msgbuf, L4_StringItem(sizeof iobuf, iobuf));
	bool running = true;
	while(running) {
		...
		for(;;) {
			...
			size_t n = fwrite(iobuf, 1, min_t(int, len, sizeof iobuf - 1), fd == 1 ? stdout : stderr);
			L4_LoadMR(0, (L4_MsgTag_t){ .X.u = 1 }.raw);
			L4_LoadMR(1, n);
			L4_AcceptStrings(L4_StringItemsAcceptor, &msgbuf);
			tag = L4_Ipc(inf->tid, inf->tid, L4_Timeouts(L4_ZeroTime, L4_Never), &(L4_ThreadId_t){ });
		}
	}
...
static struct systest *parse_stdesc(void *tal, const char *prefix, const char *group, const union stdesc *d, L4_ThreadId_t tid)
{
	struct systest *st = talloc_size(tal, sizeof *st + sizeof(struct selftest_info));
	*st = (struct systest){
		.prefix = talloc_strdup(st, prefix), .group = talloc_strdup(st, group), .name = talloc_strndup(st, d->body.name, 20),
		.low = (d->body.meta >> 2) & 0xff, .high = (d->body.meta >> 10) & 0xffff, .pri = (d->body.meta >> 26) & 0xff,
...
static bool query_selftests(void *tal, L4_ThreadId_t tid)
{
	...
	do {
		...
		if(L4_IpcFailed(tag) || L4_UntypedWords(tag) == 1) break; /* (error code in MR1; ignore it.) */
		...
		for(int i=0; i < n_descs; i++) {
			switch(desc[i].body.meta & 3) {
				case 0: darray_push(all_systests, parse_stdesc(tal, prefix, group, &desc[i], tid)); break;
				case 1: memcpy(prefix, desc[i].body.name, 20); prefix[20] = '\0'; continue;
				case 2: memcpy(group, desc[i].body.name, 20); group[20] = '\0'; continue;
				case 3: /* unused */ break;
			}
		}
		offset += n_descs;
...
static void get_systask_selftests(void *tal)
{
	...
	for(;;) {
		...
		int n = __proc_get_systask_threads(__uapi_tid, &skip, tidbuf, &n_tids, last_pid, skip);
		if(n != 0) { printf("%s: %s\n", __func__, stripcerr(n)); abort(); }
		if(n_tids == 0) break;
...
static struct systest **get_systests(size_t *num_p, bool want_ids)
{
	...
	if(darray_empty(all_systests)) {
		...
		for(size_t i=0; i < n_specs; i++) darray_push(all_systests, parse_systest(tal, specs[i]));
		assert(all_systests.size == n_specs);
		get_systask_selftests(tal);
		qsort(all_systests.item, all_systests.size, sizeof(struct systest *), &cmp_systest_ptr);
	}
...
static bool announce(const char **pfx_p, const char **grp_p, const struct systest *t)
{
...
void run_systest_by_spec(char **specs, size_t n_specs)
{
	...
	struct htable by_id = HTABLE_INITIALIZER(by_id, &hash_systest_by_id, NULL);
	...
	for(size_t i=0; i < n_specs; i++) {
		...
		struct systest *t = htable_get(&by_id, hash_string(spec), &cmp_str_to_systest_id, spec);
		if(t == NULL) {
...
void describe_all_systests(void)
{
	...
	for(size_t i=0; i < n_sts; i++) {
		if(i == 0 || !streq(sts[i]->prefix, sts[i-1]->prefix)) printf("*** desc suite `%s'\n", sts[i]->prefix);
		if(i == 0 || !streq(sts[i]->group, sts[i-1]->group)) printf("*** desc tcase `%s'\n", sts[i]->group);
